...
class Header extends Component {
  ...
  onSearch = () => {
    const {onGetPostersDetails} = this.props
    const {searchValue} = this.state
    if (searchValue !== '') {
      onGetPostersDetails(searchValue)
    }
  }
  ...
  render() {
    ...
    const {search, isHome, isPopular, isAccount} = this.props
    const searchContainer = search
      ? 'search-input-route-container search-input-container'
      : 'search-input-container'
    const searchIcon = search ? 'icons search-route-icon' : 'icons'
